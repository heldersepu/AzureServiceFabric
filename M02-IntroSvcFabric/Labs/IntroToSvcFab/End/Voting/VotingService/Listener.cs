...
public sealed class HttpCommunicationListener : ICommunicationListener
{
    ...
    public HttpCommunicationListener(StatefulServiceContext context, string endpointName, Func<HttpListenerContext, CancellationToken, Task> processRequest)
    {
        m_processRequest = processRequest;
        // Partition replica's URL is the node's IP, desired port, PartitionId, ReplicaId, Guid
        EndpointResourceDescription internalEndpoint = context.CodePackageActivationContext.GetEndpoint(endpointName);
